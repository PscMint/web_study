...
3. 使用`{}.toString.call(obj)`可以直接判断某对象属于原始类型和内建类中的哪一种，之所以要将toString借出是因为`obj.toString`规定了一些类型的特定转换方式，比如number转成string
   ...
   alert({}.toString.call(new Animal()))//[object Animal]
   ```
